...
test_eval_vip_bigger1 <- evaluate_model(data = test_predobs_vip_bigger1, 
  obs = starch, pred = pls_starch) %>% 
  # Modify columns for plot annotation
  mutate(
    rmse = as.character(as.expression(paste0("RMSE == ", "~",
...
p_test_eval_vip_bigger1 <- ggplot(data = test_predobs_vip_bigger1,
  ...
  coord_fixed(ratio = 1) +
  facet_wrap(~ eval_type) +
  geom_text(data = test_eval_vip_bigger1,
    aes(x = Inf, y = -Inf, label = r2), size = 3,
      hjust = 1.27, vjust = -3.5, parse = TRUE) +
  geom_text(data = test_eval_vip_bigger1,
    aes(x = Inf, y = -Inf, label = rmse), size = 3,
...
p_test_eval_vip_bigger1_pdf <- ggsave(filename = "test-eval-vip-bigger1.pdf",
...
p_eval_training_test_vip_bigger1 <- cowplot::plot_grid(
  p_training_eval_vip_bigger1, p_test_eval_vip_bigger1,
  ncol = 2) +
  draw_label(expression(paste("Measured starch [", mg~g^-1, " DM]")), 
    x = 0.55, y = 0, vjust = -0.3, angle = 0, size = 10) +
  draw_label(expression(paste("Predicted starch [", mg~g^-1, " DM]")), 
...
p_eval_vip_bigger1_pdf <- ggsave(filename = "eval-vip-bigger1.pdf",
